...
async function importAddressesForTesting(
  client: Client,
  addresses: string[],
): Promise<void> {
  // For Bitcoin Core v28.1 with descriptor wallets, we don't need complex descriptor imports
  // The wallet will automatically track transactions to addresses it generates
  // We only need to import addresses that are generated by the JS wallet for testing purposes

  try {
    // Try to import addresses using importmulti for better compatibility
    ...
    await client.getMethod('jsonrpc')('importmulti', requests);
    console.log(
      `Successfully imported ${addresses.length} addresses for testing`,
    );
  } catch {
    console.warn(
      'Failed to import addresses via importmulti, trying individual imports',
    );

    // Fallback to individual address imports
    for (const address of addresses) {
      try {
        await client.getMethod('jsonrpc')('importaddress', address, '', false);
      } catch (importError) {
        console.warn(
          `Failed to import address ${address}:`,
          importError.message,
        );
      }
    }
...
async function getInput(
  ...
  // Import the address for testing purposes (Bitcoin Core v28.1 descriptor wallets)
  try {
    await importAddressesForTesting(client, [unusedAddress]);
  } catch (error) {
    console.warn('Address import failed:', error.message);
    // Continue anyway - the wallet might still work for spending
  }
