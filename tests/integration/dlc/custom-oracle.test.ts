...
describe('Custom Strategy Oracle POC numdigits=21', () => {
  ...
  it('should complete entire flow', async () => {
    ...
    console.log(
      '# CETs',
      (acceptDlcOfferResponse.dlcAccept as DlcAccept).cetAdaptorSignatures.sigs
        .length,
    );
  ...
  it('should succeed in finding outcome index from PolynomialPayoutCurvePiece Payout Group', async () => {
    ...
    // const rawDlcOffer = DlcOffer.deserialize(
    //   Buffer.from(
    ...
    const payoutCurveMaxLoss = new PolynomialPayoutCurve([
      { outcome: new BN(0), payout: new BN(Number(4984379)) },
      {
        outcome: new BN(Number(999000)),
        payout: new BN(Number(4984379)),
      },
    ]);
    ...
    const payoutCurveLoss = new PolynomialPayoutCurve([
      ...
      {
        outcome: new BN(Number(1001000)),
        payout: new BN(Number(4994367)),
      },
    ]);
    ...
    payoutFunction.payoutFunctionPieces.push({
      endPoint: {
        eventOutcome: BigInt(1001000),
        outcomePayout: BigInt(4994367),
        extraPrecision: 0,
      },
    ...
    roundingIntervals.intervals = [
      { beginInterval: BigInt(0), roundingMod: BigInt(4994) },
    ];
  ...
  describe('shift fees and rounding intervals', async () => {
    ...
    before(async () => {
      ...
      finalizer = new DualFundingTxFinalizer(
        (tempDlcOffer as DlcOffer).fundingInputs,
        fakeSPK,
        fakeSPK,
        acceptInputs,
        fakeSPK,
      ...
      console.log(
        '# CETs',
        (acceptDlcOfferResponse.dlcAccept as DlcAccept).cetAdaptorSignatures
          .sigs.length,
      );
      ...
      for (const input of (dlcOffer as DlcOffer).fundingInputs) {
      ...
      for (const input of (dlcAccept as DlcAccept).fundingInputs) {
        const inputV0 = input as FundingInput;
      ...
      aliceChangeOutput = fundTx.outputs.find(
        (output) =>
          ...
          (dlcOffer as DlcOffer).changeSpk.toString('hex'),
      );

      bobChangeOutput = fundTx.outputs.find(
        (output) =>
          output.scriptPubKey.serialize().slice(1).toString('hex') ===
          (dlcAccept as DlcAccept).changeSpk.toString('hex'),
      );
    ...
    it('should execute for highest precision outcome with breakeven', async () => {
      ...
      const bobPayoutOutput = cet.outputs.find(
        (output) =>
          output.scriptPubKey.serialize().slice(1).toString('hex') ===
          (dlcAccept as DlcAccept).payoutSpk.toString('hex'),
      );
    ...
    it('should execute for highest precision outcome with gain', async () => {
      ...
      const alicePayoutOutput = cet.outputs.find(
        (output) =>
          output.scriptPubKey.serialize().slice(1).toString('hex') ===
          (dlcOffer as DlcOffer).payoutSpk.toString('hex'),
      );
      const bobPayoutOutput = cet.outputs.find(
        (output) =>
          output.scriptPubKey.serialize().slice(1).toString('hex') ===
...
describe('Custom Strategy Oracle POC numdigits=21 split trades', () => {
  ...
  it('should succeed in creating batch dlc transaction', async () => {
    ...
    const txBuilder = new BatchDlcTxBuilder(
      dlcOffers as DlcOffer[],
      dlcAccepts.map((dlcAccept) => (dlcAccept as DlcAccept).withoutSigs()),
    );
