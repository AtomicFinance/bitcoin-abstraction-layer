...
import { LiquidRpcProvider } from '../../lib';

chai.config.truncateThreshold = 0;

describe.only('Bitcoin RPC provider', () => {
  let client: Client;
  let provider: LiquidRpcProvider;

  beforeEach(() => {
    client = new Client();
    provider = new LiquidRpcProvider({
  ...
  describe('getBlockByHash', () => {
    it('should return a block', async () => {
      ...
      expect(block).to.deep.equal({
        ...
        size: 451,
        parentHash:
          '42aea9df22bc513317d9a21d319ed102c6a36d02e823c720dbead2bbec69eebc',
        nonce: 0,
        transactions: [
