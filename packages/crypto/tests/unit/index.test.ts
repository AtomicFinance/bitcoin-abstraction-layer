...
describe('Crypto library', () => {
  describe('ensureBuffer', () => {
    ...
    it('should return buffer of a given object', () => {
      expect(crypto.ensureBuffer({ message: 'LIQ' })).to.deep.equal(
        Buffer.from(JSON.stringify({ message: 'LIQ' })),
      );
    });
  });
  ...
  describe('padHexStart', () => {
    ...
    it('should return hex with 0-prefix if length is less than 2 ', () => {
      ...
      expect(crypto.padHexStart('4c4951', 16)).to.equal(
        '000000000000000000000000004c4951',
      );
    });
  });
});
